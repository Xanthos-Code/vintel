...
def textReplace(element, newText):
    newText = "<t>" + newText + "</t>"
    newElements = []
    for newPart in BeautifulSoup(newText).select("t")[0].contents:
        ...
        for word in words:
            ...
            if uword in systemNames:                   # - direct hit on name
                ...
                for system in systemNames:
                    ...
                    if (len(uwordParts) == 2 and
                        len(systemParts) == 2 and
                        len(uwordParts[0]) > 1 and
                        len(uwordParts[1]) > 1 and
                        len(systemParts[0]) > 1 and
                        len(systemParts[1]) > 1 and
                        len(uwordParts) == len(systemParts) and
                        uwordParts[0][0] == systemParts[0][0] and 
                        uwordParts[1][0] == systemParts[1][0]):
                        foundSystems.add(systems[system])
                        formattedText = formatSystem(text, word, system)
                        textReplace(text, formattedText)
                        return True
